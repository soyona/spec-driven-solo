...
> 3. **💥 3次异常熔断线与记忆轮转瘦身**：在终端运行检查、编译或测试命令时，一旦**连续失败超过 3 次**，必须立刻停止（Stop）一切 Act 行为，向人类如实报告原始日志并挂起会话。**严禁盲目猜测修改**。当 `activeContext.md` 物理行数超过 150 行时，必须在当前 Task 结束前将旧内容冷冻切片移入 `memory-bank/archive/` 物理盲区；当 `progress.md` 中已完成任务 `[x]` 累积超过 20 项时，必须将已打勾清单归档至历史日志末尾。
